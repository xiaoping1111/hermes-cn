"""CLI终端测试 - bracketed paste timeout

【产品经理理解要点】
命令行交互界面：斜杠命令、会话管理、压缩、编辑器、状态栏、快捷键等用户体验中的bracketed paste timeout验证。
- 验证功能：bracketed paste timeout功能正确性验证
- 关键场景：核心逻辑、边界条件、错误处理
- 业务影响：bracketed paste timeout功能异常或存在安全隐患
