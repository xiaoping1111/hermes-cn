"""命令行界面测试 - 环境配置·sanitize·on·load

【产品经理理解要点】
验证命令行界面的环境配置功能
- 验证的功能: Tests for .env sanitization during load to prevent token duplication (#8908)
- 核心测试场景: load env sanitizes concatenated lines、load env normal file unchanged、env loader sanitizes before dotenv
