"""命令行界面测试 - gateway·service

【产品经理理解要点】
验证命令行界面相关功能的正确性
- 验证的功能: Tests for gateway service management helpers
- 核心测试场景: preflight accepts private socket without dbus bus、wait for user dbus socket accepts private socket、systemd install repairs outdated unit without force 等共131个场景
