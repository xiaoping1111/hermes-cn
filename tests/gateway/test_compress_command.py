"""消息网关测试 - compress·命令处理

【产品经理理解要点】
验证消息网关的命令处理功能
- 验证的功能: Tests for gateway /compress user-facing messaging
- 核心测试场景: compress command reports noop without success banner、compress command explains when token estimate rises、compress command appends warning when compression aborts 等共4个场景
