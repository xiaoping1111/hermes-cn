"""消息网关测试 - complete·path·at·消息过滤

【产品经理理解要点】
验证消息网关的消息过滤功能
- 验证的功能: Regression tests for the TUI gateway's `complete.path` handler
- 核心测试场景: at folder colon only dirs、at file colon only files、at folder bare without colon lists dirs 等共15个场景
