"""消息网关测试 - Discord平台·附件处理·download

【产品经理理解要点】
验证消息网关的Discord平台附件处理功能
- 验证的功能: Tests for Discord attachment downloads via the authenticated bot session
- 核心测试场景: returns bytes on successful read、returns none when read missing、returns none when read raises 等共14个场景
