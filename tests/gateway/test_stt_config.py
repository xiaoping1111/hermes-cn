...
【产品经理理解要点】
验证消息网关的配置管理功能
- 验证的功能: Gateway STT config tests — honor stt.enabled: false from config.yaml
- 核心测试场景: gateway config stt disabled from dict nested、load gateway config bridges stt enabled from config yaml、enrich message with transcription surfaces path when stt disabled 等共6个场景
