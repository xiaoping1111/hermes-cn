"""消息网关测试 - Telegram平台·network·平台连接

【产品经理理解要点】
验证消息网关的Telegram平台平台连接功能
- 验证的功能: Tests for Telegram polling network error recovery
- 核心测试场景: reconnect self schedules on start polling failure、reconnect does not self schedule when fatal error set、reconnect success resets error count 等共15个场景
