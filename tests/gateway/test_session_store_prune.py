"""消息网关测试 - 会话管理·存储管理·prune

【产品经理理解要点】
验证消息网关的会话管理存储管理功能
- 验证的功能: Tests for SessionStore.prune_old_entries and the gateway watcher that calls it
- 核心测试场景: prune removes entries past max age、prune uses updated at not created at、prune disabled when max age is zero 等共18个场景
