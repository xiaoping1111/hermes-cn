"""消息网关测试 - runner·startup·failures

【产品经理理解要点】
验证消息网关相关功能的正确性
- 验证的功能: 网关Runner启动失败处理
- 核心测试场景: runner stays alive for retryable startup errors、runner allows cron only mode when no platforms are enabled、runner records connected platform state on success 等共9个场景
- 业务影响: 消息网关可能出现命令丢失或平台适配错误，影响所有平台用户

─────────────────────────────────────────────────────────────────
消息网关测试 - runner·startup·failures

测试多平台消息接入与命令分发中runner相关的startup相关的failures功能
"""
