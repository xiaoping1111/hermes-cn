"""测试 - gateway streaming nested config

【产品经理理解要点】
功能验证中的gateway streaming nested config验证。
- 验证功能：gateway streaming nested config功能正确性验证
- 关键场景：核心逻辑、边界条件、错误处理
- 业务影响：gateway streaming nested config功能异常或存在安全隐患
