...
【产品经理理解要点】
提供"文生视频、图生视频、视频编辑、视频续拍"四合一入口，后端由插件驱动，用户按需开通。
- 核心职责：将多种视频生成能力（文本→视频、图片→视频、编辑、续拍）统一为一个工具接口，LLM调用时无需关心底层差异
- 关键业务概念：插件化后端——本项目不内置任何视频生成供应商，需通过 hermes plugins enable 启用对应插件
- 在系统中的位置：与 image_generate 镜像对称设计，属于多模态生成工具族的一环
