...
【产品经理理解要点】
集中判断"该用哪个后端"的通用逻辑，如浏览器是本地还是云端、Nous 付费网关是否可用等。
- 核心职责：检查 Nous 订阅状态、浏览器云端提供商选择、Modal 执行模式选择、音频 API Key 解析
- 关键业务概念：统一决策点——所有工具共享同一个"付费用户否"判断和"后端选择"逻辑，避免各处重复不一致
- 在系统中的位置：各类工具在初始化时调用的基础设施层
