...
【产品经理理解要点】
支持将任务委派给子智能体在后台执行，子智能体完成后通知主智能体继续处理。
- 核心职责：管理后台子智能体的创建、运行和完成通知，复用进程注册表的completion_queue事件队列
- 业务价值：用户可以在子智能体工作时继续与主智能体对话，完成后自动触发新对话轮次
- 在系统中的位置：位于委派工具和进程管理系统之间，是并行任务执行的核心组件
