...
【产品经理理解要点】
让 Agent 在元宝（腾讯对话机器人平台）上查询群信息、发送贴纸和私聊消息，是元宝平台交互的核心工具。
- 核心职责：查询群基本信息/成员、搜索和发送贴纸、发送私聊消息
- 关键业务概念：贴纸工作流——先 search_sticker 搜索贴纸 ID，再 send_sticker 发送，避免裸 emoji 误用
- 在系统中的位置：hermes-yuanbao 工具集的 Agent 侧入口，通过活跃适配器单例与元宝平台通信
