...
【产品经理理解要点】
让智能体具备"看图"能力，支持从URL或本地路径加载图片并进行分析。
- 核心职责：图片下载、格式检测、尺寸自动调整、调用视觉模型分析图片内容
- 双通道架构：原生视觉通道（主模型直接看到图片像素，零信息损失）和辅助LLM通道（用独立视觉模型描述图片，返回文字）
- 安全机制：SSRF防护（阻止私网地址请求）、图片大小限制（防OOM）、网站访问策略检查、重定向安全校验
- 在系统中的位置：位于用户图片交互和视觉AI模型之间，是智能体多模态感知能力的关键组件
