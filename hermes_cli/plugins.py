...
【产品经理理解要点】
发现、加载和管理插件，支持工具注册、生命周期钩子、平台适配器等多种扩展机制。
- 核心职责：从四个来源（内置/用户/项目/pip）扫描插件，按优先级加载并注册工具、钩子、中间件、命令等
- 关键概念：插件清单（plugin.yaml）、生命周期钩子（pre_tool_call/post_tool_call 等）、插件上下文（PluginContext）、安全模式下跳过加载
- 系统定位：Hermes 的扩展核心，所有第三方能力和部分内置能力（图像生成、语音合成、浏览器等）都通过插件注册
