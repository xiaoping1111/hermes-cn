...
【产品经理理解要点】
控制"谁能在飞书文档评论区与 Agent 对话"的权限系统。
- 核心职责：决定哪些用户可以在飞书文档评论中触发 Agent 回复，保障信息安全
- 权限策略：支持两种模式——"allowlist"（白名单，仅指定用户可评论）和"pairing"（配对，用户扫码后自动获批）
- 规则层级：可按单个文档精确配置，也可用通配符"*"统一管理，未配置时继承上层默认值
- 配置热更新：修改 JSON 配置文件后无需重启服务，下次评论事件自动生效
