...
【产品经理理解要点】
管理员首次接入 QQ 机器人时的"扫码绑定"引导流程，零配置文件即可完成初始化。
- 核心职责：生成二维码 → 管理员手机扫码 → 自动获取机器人凭证，免去手动填写 AppID 和密钥
- 操作流程：①终端显示二维码 → ②管理员用手机 QQ 扫码确认 → ③自动获取并解密凭证
- 安全保障：凭证经 AES-256 加密传输，仅在本地解密，密钥不在网络中明文传递
- 超时容错：二维码过期后自动刷新（最多3次），总超时10分钟
