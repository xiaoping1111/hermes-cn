...
【产品经理理解要点】
让 Agent 能在元宝（腾讯元宝）对话中发送图片和文件。
- 核心职责：处理图片/文件的上传与消息格式化，是 Agent 多模态能力的底层支撑
- 上传流程：先获取腾讯云 COS 临时凭证 → 用凭证签名上传文件 → 构建消息体发送
- 业务场景：当用户要求 Agent 生成图片、发送文件、返回 PDF 等时，本模块负责将内容送达元宝客户端
- 技术选型：纯 HTTP 实现，不依赖 COS SDK，降低部署复杂度
