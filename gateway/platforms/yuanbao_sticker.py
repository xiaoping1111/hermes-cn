...
【产品经理理解要点】
让 Agent 能在元宝对话中发送表情贴纸，增强交互趣味性和情感表达。
- 核心职责：管理内置贴纸库、按关键词/语义模糊搜索匹配贴纸、构建腾讯 IM 贴纸消息格式
- 贴纸库：内置 50+ 款表情（如"666""狗头""吃瓜""贴贴"等），覆盖日常互动场景
- 智能匹配：支持精确名称、关键词、同义词（如输入"厉害"可匹配"六六六"）和模糊评分排序
- 业务场景：当用户输入"发个表情"或对话上下文适合贴纸回应时触发
