"""网关会话管理 — 追踪对话来源、持久化存储、自动重置策略

【产品经理理解要点】
网关需要管理多个平台上的对话会话，这个模块负责：
  - 会话追踪：记录消息来自哪个平台、哪个频道、哪个用户
  - 会话存储：将对话持久化到磁盘，重启后不丢失
  - 重置策略：对话太长或太久时自动重新开始
  - 上下文注入：告诉 AI "你在 Telegram 某群里和某用户对话"

就像酒店前台管理系统——记录每位客人的来源、房间号、入住时间。
