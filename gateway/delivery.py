"""消息投递路由 — 将定时任务输出和 AI 回复发到正确的频道

【产品经理理解要点】
网关需要把消息投递到正确的目的地，这个模块负责路由：
  - 指定目标：如 "telegram:123456789" 直接发到指定频道
  - 平台主频道：如 "telegram" 发到该平台的默认频道
  - 来源回复：定时任务的结果发回创建它的地方
  - 本地保存：所有输出同时保存到文件

就像邮件系统的"分拣中心"——根据地址把信投到正确的信箱。
