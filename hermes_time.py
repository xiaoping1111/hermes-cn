...
【产品经理理解要点】
统一时间服务：根据用户配置的时区返回正确的时间，确保定时任务和日志时间戳准确。
- 核心职责：解析用户时区设置（环境变量→配置文件→系统默认）、缓存时区对象、提供 now()
- 关键业务概念：IANA 时区（如 Asia/Shanghai）、定时任务调度依赖、配置优先级
- 在系统中的位置：被 cron 定时任务、日志模块等需要时间戳的模块调用
