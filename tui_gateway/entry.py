...
【产品经理理解要点】
TUI 网关的启动入口，初始化模块路径、信号处理、MCP 发现线程，并启动 JSON-RPC 分发循环。
- 核心职责：设置 Python 模块搜索路径防止本地包遮挡，安装侧边栏 WS 发布器，运行主分发循环
- 关键概念：支持 Dashboard 侧边栏事件推送（通过 HERMES_TUI_SIDECAR_URL），MCP 工具在后台线程发现
- 系统定位：启动引导层，将 TUI 前端的 JSON-RPC 请求路由到 server.dispatch 处理
