...
【产品经理理解要点】
TUI模式下Hermes的核心服务进程，管理Agent会话、处理用户交互、协调工具调用。
- 核心职责：实现所有JSON-RPC方法，管理会话生命周期，处理斜杠命令和审批流程
- 会话管理：创建/恢复/压缩/分叉会话，支持模型切换和历史压缩
- 异步调度：慢操作（斜杠命令、Shell执行等）在线程池中处理，不阻塞主通信
- 审批流程：通过前端弹窗让用户确认危险命令和文件编辑
- 崩溃保护：未捕获异常自动记录到崩溃日志，方便问题排查
"""
import atexit
