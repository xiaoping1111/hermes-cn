"""网页搜索注册表 — 管理可用的网页搜索和提取服务

【产品经理理解要点】
Agent 可以搜索网页和提取网页内容，但搜索服务有多个提供商（Tavily、Exa、
Brave、DuckDuckGo、SearXNG 等）。这个模块管理所有已注册的搜索服务：

  - 搜索和提取可分别配置不同的后端
  - 用户可在 config.yaml 中指定首选提供商
  - 如果未指定，按优先级自动选择（firecrawl → tavily → exa → ...）
  - 新的搜索服务可通过插件注册
