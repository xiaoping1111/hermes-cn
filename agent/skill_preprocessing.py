...
【产品经理理解要点】
技能文件（SKILL.md）支持"模板语法"，比如用 ${HERMES_SKILL_DIR} 表示技能目录路径，
或者用 !`date +%Y-%m-%d` 自动插入当前日期。这个模块负责在加载技能时做这些替换：

  - 模板变量替换：把 ${HERMES_SKILL_DIR} 替换为技能实际所在的目录路径
  - 内联命令执行：把 !`命令` 替换为命令的执行结果

为什么需要预处理？因为技能可以是通用的模板，不同用户安装在不同路径下，
需要动态替换路径等变量才能正确使用。
