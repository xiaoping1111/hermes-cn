...
【产品经理理解要点】
定义图像生成的插件化后端接口，统一文生图和图生图两种模式。
- 核心职责：定义 ImageGenProvider ABC(generate)、文生图/图生图路由
- 关键业务概念：文生图(text-to-image)、图生图(image-to-image)、模型选择
- 在系统中的位置：image_generate 工具的供应商抽象层
