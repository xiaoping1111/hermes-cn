...
【产品经理理解要点】
为受信插件提供受控的 LLM 调用能力，让插件也能使用 AI 模型完成工作。
- 核心职责：插件通过 ctx.llm 调用聊天补全和结构化推理，无需自行管理模型和密钥
- 安全机制：宿主控制路由、认证和超时；插件不可见原始密钥，覆写需显式授权
- 业务场景：错误改写、网关适配、摘要生成、定时评分等插件需要独立调用模型的场景
