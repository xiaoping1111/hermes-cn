"""上下文压缩器 — 长对话自动压缩，防止超出模型记忆窗口

【产品经理理解要点】
AI 模型有"记忆容量上限"（就像人的短期记忆），当对话太长时会装不下。
这个模块就像一个智能笔记员，在对话快满时自动做摘要压缩：

  - 保护头部：系统指令 + 第一轮对话（最重要，包含任务设定）绝不压缩
  - 保护尾部：最近几轮对话（当前正在讨论的内容）绝不压缩
  - 智能压缩中间部分：用便宜的辅助模型把中间对话总结成摘要
  - 摘要质量高：保留已解决的问题、待处理的事项、关键决策，不丢失信息
  - 迭代压缩：多次压缩时会基于之前的摘要继续更新，而非从头开始

为什么要压缩而不是直接截断？
  截断会丢信息，AI 会重复问已经回答过的问题；压缩保留关键信息，
  AI 仍能"记住"之前的讨论要点，只是细节被浓缩了。

压缩示意图：
  ┌─────────────────────────────────────────────┐
  │ 系统指令 + 首轮对话（保护，不压缩）          │ ← 头部
  ├─────────────────────────────────────────────┤
  │ [压缩摘要：之前讨论了X，解决了Y，待做Z]     │ ← 摘要
  ├─────────────────────────────────────────────┤
  │ 最近 3-5 轮完整对话（保护，不压缩）          │ ← 尾部
  └─────────────────────────────────────────────┘
