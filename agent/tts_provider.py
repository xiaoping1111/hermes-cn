...
【产品经理理解要点】
定义语音合成(TTS)的插件化后端接口，支持多种语音引擎切换。
- 核心职责：定义 TTSProvider ABC(synthesize/list_voices)、三级优先级(内置-命令-插件)
- 关键业务概念：TTS 后端、内置优先、命令类型供应商、流式音频
- 在系统中的位置：text_to_speech 工具的供应商抽象层
