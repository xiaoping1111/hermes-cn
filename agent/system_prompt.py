"""系统提示词组装 — 构建 AI 的"人格与行为规范"

【产品经理理解要点】
系统提示词是发送给 AI 模型的"角色说明书"，决定了 AI 是谁、怎么做事。
这个模块负责把各种规则和背景信息组装成一个完整的系统提示词。
...
系统提示词分三层（从稳定到易变）：
  ...
  │   - 工具使用指南（什么时候该用什么工具）              │
  │   - 技能提示（当前加载了哪些技能）                    │
  │   - 模型专用指导（不同 AI 模型有不同的注意事项）       │
  ├─────────────────────────────────────────────────────┤
  │ 上下文层 (context)：                                │
  │   - 项目说明文件（AGENTS.md 等项目级配置）           │
  │   - 调用方传入的自定义系统消息                       │
  ├─────────────────────────────────────────────────────┤
  │ 易变层 (volatile)：                                 │
  │   - 用户画像（用户偏好、习惯）                       │
  ...
  为什么分层？因为稳定层很少变，可以让 AI 提供商做缓存加速；
  易变层每次都可能变，放在一起会影响缓存效果。
