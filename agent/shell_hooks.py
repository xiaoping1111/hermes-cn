"""Shell 脚本钩子 — 允许用户用脚本扩展 Agent 的行为

【产品经理理解要点】
这个模块让用户可以在 Agent 的关键节点上运行自己的脚本，定制 Agent 的行为。

可用的事件钩子：
  - pre_tool_call：工具执行前（如命令执行前确认）
  - post_tool_call：工具执行后（如执行完命令后发通知）
  - 其他自定义事件

安全机制：
  - 首次使用时需要用户确认每个脚本（防止恶意脚本）
  - 脚本直接执行，不经过 shell 解析（防止注入攻击）
  - 确认记录保存在白名单文件中，后续自动放行

就像浏览器扩展：用户可以在特定时机插入自己的逻辑，但需要先授权。
